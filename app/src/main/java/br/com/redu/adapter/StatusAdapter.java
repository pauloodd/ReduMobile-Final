...
import br.com.redu.gui.ActivityStatusedByLectureDetailsActivity;
import br.com.redu.gui.ActivityStatusedBySpaceDetailsActivity;
import br.com.redu.gui.LectureWallActivity;
import br.com.redu.gui.LinkEnabledTextView;
import br.com.redu.gui.SpaceWallActivity;
import br.com.redu.util.BitmapManager;
import br.com.redu.util.DateFormatter;
import br.com.redu.util.DownloadImageTask;
...
import br.com.redu.gui.ActivityStatusedByUserDetailsActivity;
import br.com.redu.gui.HelpStatusedByLectureDetailsActivity;
import br.com.redu.gui.UserWallActivity;
import br.com.redu.gui.clickablespan.BlueClickableSpan;
import br.com.redu.gui.clickablespan.BoldBlueClickableSpan;
...
public final class StatusAdapter extends BaseAdapter {
	...
	private final class ActivityStatusedByUserViewHolder {
		...
		private TextView lblInfo;
		private LinkEnabledTextView lblText;
		private LinearLayout boxAnswer;
	}
	...
	private LinkEnabledTextView.TextLinkClickListener textLinkClickListener;
	...
	public StatusAdapter(Context context, ArrayList<Status> statuses) {
		...
		textLinkClickListener = new LinkEnabledTextView.TextLinkClickListener() {
			@Override
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (viewType == VIEW_TYPE_ACTIVITY_STATUSED_BY_USER) {
			...
			if (convertView == null) {
				...
				holder.boxAnswer = (LinearLayout) viewGroup.findViewById(R.id.activityStatusedByUserLytText);
				holder.imgUpArrow = (ImageView) viewGroup.findViewById(R.id.activityStatusedByUserImgUpArrow);
			...
			if(statuses.get(position) instanceof Activity){
				montarViewActivity(position, holder);
			}else{
				montarViewLog(position, holder);
			}
			...
			new DownloadImageTask(holder.imgUserThumb).execute(thumbUrl);
	...
	private void montarViewActivity(int position, ActivityStatusedByUserViewHolder holder) {
		final Activity activity = (Activity) statuses.get(position);
	...
	private void montarViewLog(int position, ActivityStatusedByUserViewHolder holder) {
		final Log log = (Log) statuses.get(position);

		holder.lblCreationDate.setText(DateFormatter.getDataPostagemEmPalavras(log
				.getCreatedAt()));

		SpannableString info = null;

		final User activityUser = log.getUser();
		...
		String infoText = activityUserFullName+ " "+log.getText();
		if(log.getText() != null && log.getText().contains("criou a Disciplina")){
			infoText += "";
		}
		...
		holder.imgArrowRevolve.setVisibility(View.GONE);
		holder.imgAnswers.setVisibility(View.GONE);
		holder.lblAnswer.setVisibility(View.GONE);
		holder.lblAnswersCount.setVisibility(View.GONE);
		holder.lblText.setVisibility(View.GONE);
		holder.boxAnswer.setVisibility(View.GONE);
